...
# The output is an Nx128 array of SIFT descriptors
    ...
    for i in range(angles.shape[0]):
        kpts.append(cv2.KeyPoint(circles[i, 0], circles[i, 1],size=enlarge_factor*circles[i, 2],angle=angles[i]))
...
if __name__ == '__main__':
    ...
    I1 = imread("../data/stitching/eg_1.jpg")
    I2 = imread("../data/stitching/eg_2.jpg")
    circles1 = [[kp1[i].pt[0], kp1[i].pt[1], kp1[i].size, kp1[i].angle] for i in range(len(kp1))]
    circles1 = np.array(circles1)
    circles2 = [[kp2[i].pt[0], kp2[i].pt[1], kp2[i].size, kp1[i].angle] for i in range(len(kp2))]
